<div align="center">
<h1>YOLOv26 Triple Input</h1>
<h3>YOLOv26: Enhanced Multi-Image Object Detection with Triple Input Architecture</h3>

**Research Group, Department of Civil Engineering**  
**King Mongkut's University of Technology Thonburi (KMUTT)**

Original YOLOv26 by [Yunjie Tian](https://sunsmarterjie.github.io/)<sup>1</sup>, [Qixiang Ye](https://people.ucas.ac.cn/~qxye?language=en)<sup>2</sup>, [David Doermann](https://cse.buffalo.edu/~doermann/)<sup>1</sup>
...
<p align="center">
  <img src="assets/yolov26_triple_dinov3_architecture.svg" width=100%> <br>
  YOLOv26 Triple Input + DINOv3 Architecture for Enhanced Civil Engineering Applications
</p>
...
- **🔄 Backward Compatible**: Seamlessly works with existing YOLOv26 infrastructure
...
- 2025/06/17: **Use this repo for YOLOv26 instead of [ultralytics](https://github.com/ultralytics/ultralytics/tree/main/ultralytics/cfg/models/12). Their implementation is inefficient, requires more memory, and has unstable training, which are fixed here!**
- 2025/07/01: YOLOv26's **classification** models are released, see [code](https://github.com/sunsmarterjie/yolov26/tree/Cls).
- 2025/06/04: YOLOv26's **instance segmentation** models are released, see [code](https://github.com/sunsmarterjie/yolov26/tree/Seg).

<details>
  <summary>
  <font size="+1">Abstract</font>
  </summary>
Enhancing the network architecture of the YOLO framework has been crucial for a long time but has focused on CNN-based improvements despite the proven superiority of attention mechanisms in modeling capabilities. This is because attention-based models cannot match the speed of CNN-based models. This paper proposes an attention-centric YOLO framework, namely YOLOv26, that matches the speed of previous CNN-based ones while harnessing the performance benefits of attention mechanisms.

YOLOv26 surpasses all popular real-time object detectors in accuracy with competitive speed. For example, YOLOv26-N achieves 40.6% mAP with an inference latency of 1.64 ms on a T4 GPU, outperforming advanced YOLOv10-N / YOLOv11-N by 2.1%/1.2% mAP with a comparable speed. This advantage extends to other model scales. YOLOv26 also surpasses end-to-end real-time detectors that improve DETR, such as RT-DETR / RT-DETRv2: YOLOv26-S beats RT-DETR-R18 / RT-DETRv2-R18 while running 42% faster, using only 36% of the computation and 45% of the parameters.

**Triple Input Enhancement for Civil Engineering**: This repository, developed by the Research Group at the Department of Civil Engineering, KMUTT, extends YOLOv26 with multi-image processing capabilities specifically optimized for civil engineering applications. The enhancement allows the model to leverage contextual information from multiple related images for improved detection accuracy in infrastructure monitoring, structural analysis, and construction site management.
</details>

## Main Results

### Standard YOLOv26 (Single Input)
**Turbo (default)**:
| Model (det)                                                                              | size<br><sup>(pixels) | mAP<sup>val<br>50-95 | Speed (ms) <br><sup>T4 TensorRT10<br> | params<br><sup>(M) | FLOPs<br><sup>(G) |
| :----------------------------------------------------------------------------------- | :-------------------: | :-------------------:| :------------------------------:| :-----------------:| :---------------:|
| [YOLO12n](https://github.com/sunsmarterjie/yolov26/releases/download/turbo/yolov26n.pt) | 640                   | 40.4                 | 1.60                            | 2.5                | 6.0               |
| [YOLO12s](https://github.com/sunsmarterjie/yolov26/releases/download/turbo/yolov26s.pt) | 640                   | 47.6                 | 2.42                            | 9.1                | 19.4              |
| [YOLO12m](https://github.com/sunsmarterjie/yolov26/releases/download/turbo/yolov26m.pt) | 640                   | 52.5                 | 4.27                            | 19.6               | 59.8              |
| [YOLO12l](https://github.com/sunsmarterjie/yolov26/releases/download/turbo/yolov26l.pt) | 640                   | 53.8                 | 5.83                            | 26.5               | 82.4              |
| [YOLO12x](https://github.com/sunsmarterjie/yolov26/releases/download/turbo/yolov26x.pt) | 640                   | 55.4                 | 10.38                           | 59.3               | 184.6             |

### YOLOv26 Triple Input (Multi-Image) - KMUTT Civil Engineering
...
### YOLOv26 Triple Input + DINOv3 (Enhanced) - KMUTT Civil Engineering
...
### YOLOv26 Triple Input + DINOv3 Satellite (🛰️ NEW) - KMUTT Civil Engineering
...
```bash
# Clone repository
git clone https://github.com/Sompote/Triple-dino-yolov26.git
cd Triple-dino-yolov26

# Install flash attention (optional but recommended)
wget https://github.com/Dao-AILab/flash-attention/releases/download/v2.7.3/flash_attn-2.7.3+cu11torch2.2cxx11abiFALSE-cp311-cp311-linux_x86_64.whl

# Create environment
conda create -n yolov26 python=3.11
...
### Standard YOLOv26 Usage

#### Validation
```python
from ultralytics import YOLO

model = YOLO('yolov26n.pt')  # n/s/m/l/x
...
model = load_pretrained_weights_to_triple_model(
    pretrained_path='yolov26n.pt',
    triple_model_config='ultralytics/cfg/models/v26/yolov26_triple.yaml'
)
...
python load_pretrained_triple.py \
    --pretrained yolov26s.pt \
    --model ultralytics/cfg/models/v26/yolov26_triple.yaml \
    --save yolov26s_triple_pretrained.pt
...
conv = TripleInputConv.from_pretrained(
    c1=9, c2=64, 
    pretrained_model_path='yolov26n.pt',
    k=3, s=2
)
...
class TripleInputConv(nn.Module):
    ...
    Supports loading pretrained weights from standard YOLOv26 models.
    """
    ...
    def __init__(self, c1, c2, k=3, s=1, p=None, g=1, d=1, act=True, pretrained_weights=None):
        ...
        # pretrained_weights: Optional pretrained weights from YOLOv26
    
    @classmethod
    def from_pretrained(cls, c1, c2, pretrained_model_path, **kwargs):
        """Create TripleInputConv with pretrained YOLOv26 weights."""
...
- **Pretrained Weight Support**: Compatible with YOLOv26 pretrained models
- **Transfer Learning**: Enables fine-tuning from standard YOLOv26 weights
...
| Training Strategy | From scratch or pretrained | From scratch or YOLOv26 pretrained |
...
# Test model creation
python -c "from ultralytics import YOLO; YOLO('ultralytics/cfg/models/v26/yolov26_triple.yaml'); print('Model creation successful')"
```
...
- [ultralytics/cfg/models/v26/yolov26_triple_dinov3.yaml](ultralytics/cfg/models/v26/yolov26_triple_dinov3.yaml) - DINOv3 model configuration
...
| `1` | str | `s` | YOLOv26 model variant (`n`, `s`, `m`, `l`, `x`) |
| `--save-period` | int | `-1` | Save weights every N epochs (`-1` = only best/last, saves disk space) |
| `--name` | str | `yolov26_triple_dinov3` | Experiment name for output directory |
...
| Argument | Type | Default | Description |
|----------|------|---------|-------------|
| `--pretrained` | str | `None` | Path to pretrained YOLOv26 model (.pt file) |
...
- Uses `yolov26_triple_dinov3_p0p3.yaml` configuration
- Best for: Maximum feature enhancement with dual processing

#### YOLOv26 Model Variants (`--variant`)
...
# Large YOLOv26 variant with satellite DINOv3
python train_triple_dinov3.py \
    --data dataset.yaml \
    --integrate initial \
    --dinov3-size sat_large \
...
# 🛰️ Satellite DINOv3 training with extra large YOLOv26
python train_triple_dinov3.py \
    --data satellite_dataset.yaml \
    --integrate initial \
    --dinov3-size sat_giant \
...
| `--cache-dir` | str | `~/.cache/yolov26_dinov3` | Cache directory for models |
...
### From Standard YOLOv26 to Triple Input

1. **Update model configuration**: Use `yolov26_triple.yaml` instead of `yolov26.yaml`
...
from ultralytics import YOLO
model = YOLO('ultralytics/cfg/models/v26/yolov26_triple_dinov3.yaml')
model.train(
    data='your_dataset.yaml',
    epochs=100,
...
This project is licensed under the AGPL-3.0 License - see the original YOLOv26 license.
...
- Original YOLOv26 by [Yunjie Tian](https://sunsmarterjie.github.io/) et al.
...
```bibtex
@article{tian2025yolov26,
  title={YOLOv26: Attention-Centric Real-Time Object Detectors},
  author={Tian, Yunjie and Ye, Qixiang and Doermann, David},
  journal={arXiv preprint arXiv:2502.12524},
  year={2025}
}

@misc{yolov26_triple_input,
  title={YOLOv26 Triple Input with DINOv3 Integration for Civil Engineering Applications},
...
[🤖 DINOv3 Training](train_triple_dinov3.py) | [📥 Download DINOv3](download_dinov3.py) | [📚 Documentation](TRIPLE_INPUT_README.md) | [🔧 Pretrained Training](train_triple_pretrained.py) | [⚙️ Configuration](ultralytics/cfg/models/v26/yolov26_triple_dinov3.yaml)

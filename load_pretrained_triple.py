#!/usr/bin/env python3
"""
Utility script for loading pretrained YOLOv26 weights into triple input models.

This script provides utilities to:
1. Load standard YOLOv26 pretrained weights
2. Initialize triple input models with pretrained weights
3. Fine-tune triple input models from pretrained checkpoints

Usage:
    python load_pretrained_triple.py --pretrained yolov26n.pt --model ultralytics/cfg/models/v26/yolov26_triple.yaml
    python load_pretrained_triple.py --pretrained yolov26s.pt --model ultralytics/cfg/models/v26/yolov26_triple.yaml --save yolov26s_triple_pretrained.pt
"""
...
def load_pretrained_weights_to_triple_model(pretrained_path, triple_model_config, save_path=None):
    """
    Load pretrained YOLOv26 weights into a triple input model.
    
    Args:
        pretrained_path (str): Path to pretrained YOLOv26 model (.pt file)
...
def main():
    parser = argparse.ArgumentParser(description='Load pretrained YOLOv26 weights into triple input model')
    ...
    if args.save:
        ...
        print(f"  model.save('yolov26_triple_pretrained.pt')")
        print(f"  model.train(data='test_triple_dataset.yaml', epochs=100)")
